/**
 * TikTok Shop Channel Management API
 *
 * GET  /api/channels/tiktok          — List all TikTok channels
 * POST /api/channels/tiktok          — Initiate OAuth connection
 */
...
import { getOAuthUrl } from "@/lib/tiktok-shop";
import { env } from "@/lib/env";
import { z } from "zod";
import { audit } from "@/lib/audit";
import crypto from "crypto";

const REDIRECT_URI = `${env.NEXT_PUBLIC_APP_URL}/api/channels/tiktok/callback`;

// GET — List all TikTok sales channels
export async function GET() {
  ...
  const channels = await db.query.salesChannels.findMany({
    where: eq(salesChannels.platform, "TIKTOK"),
    columns: {
      id: true,
      name: true,
      platform: true,
...
export async function POST(req: Request) {
  ...
  if (!env.TIKTOK_APP_KEY || !env.TIKTOK_APP_SECRET) {
    return NextResponse.json(
      { error: "TikTok Shop API credentials not configured. Set TIKTOK_APP_KEY and TIKTOK_APP_SECRET." },
      { status: 400 },
    );
  }

/**
 * WhatsApp Catalog Sync Trigger
 *
 * POST /api/channels/whatsapp/[id]/sync — Trigger full catalog sync
 */
...
import { syncWhatsAppCatalog } from "@/lib/whatsapp-catalog-sync";
...
export async function POST(req: Request, ctx: RouteContext) {
  ...
  const channel = await db.query.salesChannels.findFirst({
    ...
    columns: { id: true, name: true, externalCatalogId: true, externalAccountId: true, status: true },
  });
  ...
  try {
    const result = await syncWhatsAppCatalog(id);

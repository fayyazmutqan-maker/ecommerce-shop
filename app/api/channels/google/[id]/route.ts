/**
 * Individual Google Channel API
 *
 * GET    /api/channels/google/[id] — Get channel details + sync stats
 * PATCH  /api/channels/google/[id] — Update settings (merchant ID, sync config)
 * DELETE /api/channels/google/[id] — Disconnect and remove channel
 */
...
const updateSchema = z.object({
  ...
  settings: z.object({
    ...
    contentLanguage: z.string().min(2).max(5).optional(),
...
export async function PATCH(req: Request, ctx: RouteContext) {
  ...
  if (parsed.data.merchantId) updates.externalAccountId = parsed.data.merchantId;
  ...
  audit({
    ...
    resourceId: id,
    details: { platform: "GOOGLE", updates: Object.keys(parsed.data) },
    success: true,
  });
